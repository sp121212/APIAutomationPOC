pipeline {
   ...
   stages {
      ...
      stage('Run Tests with - Docker Images') {
         steps {
           script{
           def exitCode = bat (script: "docker run --name apitest${BUILD_NO} -e MAVEN_OPTS='-Dsurefire.suiteXmlFiles=src/test/resources/testrunners/SanityTest.xml' santanu1212/test_v1:latest", returnStatus: true )
           	if(exitCode != 0){
