pipeline {
   ...
   stages {
      ...
      stage('Run Tests with - Docker Images') {
         steps {  	
          script{
           def exitCode = sh(script: "docker run --name apitest_${BUILD_NO} -e MAVEN_OPTS='-Dsurefire.suiteXmlFiles=src/test/resources/testrunners/SanityTest.xml' santanu1212/test13:latest", returnStatus = true )
           	if(exitCode != 0){
